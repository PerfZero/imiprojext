...
// Состояния для управления данными пополнения
...
const handleTopup = async () => {
    ...
    try {
        ...
        const response = await apiService.walletDeposit(requestData);

        // При успешном ответе показываем уведомление
        // showToast('Успех', `Баланс пополнен на ${amount.value} ${currency.value}!`, 5000);
    } catch (error) {
        ...
        if (error && error.error) {
            ...
            errorMessage.value = 'Не удалось пополнить баланс. Попробуйте ещё раз.';
        }
                ...
                <div class="col-12 col-md-7 col-lg-6 col-xl-5 col-xxl-4 mb-auto">
                    ...
                    <ul class="list-group adminuiux-list-group">
                        <li class="list-group-item">
                            ...
                            <label class="form-check-label stretched-link" for="firstRadio2">Перевод на карту</label>
                        </li>
                        <li class="list-group-item">
                            <input v-model="paymentMethod" class="form-check-input me-1" type="radio"
                    ...
                    <div class="col-12 col-md-auto mx-auto mb-3 mb-lg-4 mt-auto">
                        <button @click="handleTopup" class="btn btn-theme w-10">Пополнить</button>
                    </div>
                </div>
            </div>
        </div>
